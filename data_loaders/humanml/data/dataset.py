...
class TextOnlyDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        if use_lmbdb:
            # speed up loading using the lmdb file instead
            import lmdb
            lmdb_file = "dataset/HumanML3D/text_lmdb"
            # open lmdb file and read entries
            ...
            with env.begin() as txn:
                for name in tqdm(id_list):
                    try:
                        ...
                        value = txn.get(name.encode()).decode() 
...
def sample_to_motion(sample_abs, dataset, model, abs_3d=True):
    n_joints = 22
    # (bs, 263, 1, 120)
    ...
    sample = recover_from_ric(sample, n_joints, abs_3d=abs_3d)
