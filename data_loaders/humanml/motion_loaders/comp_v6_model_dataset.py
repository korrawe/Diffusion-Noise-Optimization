...
from sample.condition import (cond_fn_key_location, get_target_from_kframes, get_target_and_inpt_from_kframes_batch, 
                              ...
                              CondKeyLocations, CondKeyLocationsWithSdf, CondKeyLocationsLoss)
...
# Data class for generated motion by *conditioning*
class CompGeneratedDatasetPoseCondition(Dataset):

    def __init__(self, args, model_dict, diffusion_dict, dataloader, mm_num_samples, mm_num_repeats, 
                 max_motion_length, num_samples_limit, scale=1., save_dir=None, seed=None, noise_opt_conf: NoiseOptOptions=None):
        ...
        model, diffusion_ori = model_dict["motion"], diffusion_dict["motion"]

        ### Basic settings
        log_motion = True # False
        log_idx_vis = 5
        
        model_device = next(model.parameters()).device
        diffusion_ori.data_get_mean_fn = self.dataset.t2m_dataset.get_std_mean
        diffusion_ori.data_transform_fn = self.dataset.t2m_dataset.transform_th
        diffusion_ori.data_inv_transform_fn = self.dataset.t2m_dataset.inv_transform_th
        ...
        for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
            ...
            # # Visualize to make sure it is correct
            # if log_motion:
            #     plot_3d_motion(os.path.join(self.save_dir, f"{motion_id}_gt.mp4"), self.dataset.kinematic_chain, 
            #                    gt_skel_motions[log_idx_vis].permute(2,0,1).detach().cpu().numpy(), 'title', 'humanml', fps=20)
            ...
            for t in range(repeat_times):
                ...
                batch_file = f'{i:04d}_{t:02d}.pt'
                batch_path = os.path.join(self.save_dir, batch_file)
                batch_path_first_gen = os.path.join(self.save_dir, f'{i:04d}_{t:02d}_first_gen.pt')

                # reusing the batch if it exists
                if os.path.exists(batch_path):
                    # [bs, njoints, nfeat, seqlen]
                    final_motion = torch.load(batch_path, map_location=motion.device)
                    first_gen_motion = torch.load(batch_path_first_gen, map_location=motion.device)
                    print(f'batch {batch_file} exists, loading from file')
                else:                        
                    print(f'working on {batch_file}')
                    # for smoother motions
                    ...
                    if log_motion:
                        for j in [3]:
                            vis_len = model_kwargs['y']['lengths'][j].cpu().numpy()
                            cap = model_kwargs['y']['text'][j]
                            motion_id = f'{i:04d}_{t:02d}_{j:02d}'
                            cur_motion = sample_to_motion(sample, self.dataset, model, abs_3d=args.abs_3d)
                            plot_3d_motion(os.path.join(self.save_dir, f"{motion_id}_first_gen.mp4"), self.dataset.kinematic_chain, 
                                    cur_motion[j].permute(2,0,1)[:vis_len].detach().cpu().numpy(), cap, 'humanml', fps=20)
                    # save first gen
                    torch.save(sample, batch_path_first_gen)
                    first_gen_motion = sample.clone()

                    # 2. Do DDIM inversion
                    model_kwargs_inversion = copy.deepcopy(model_kwargs)
                    model_kwargs_inversion["y"]["text"] = [""]
                    inv_noise = diffusion_ori.invert(
                        model,
                        sample.clone().float().to(model_device),
                        model_kwargs=model_kwargs_inversion,  # model_kwargs['y']['text'],
                        dump_steps=[], # dump_steps,
                        num_inference_steps=99 # 49, # 99,
                    )
                    inv_noise_detach = inv_noise.detach().clone()

                    if noise_opt_conf.opt_steps == 0:
                    ...
                    if DO_OPTIMIZATION:
                        # 3. Optimize from the initial noise
                        # Assume the batch size is 32. Split the batch into 8 batches of 4.
                        # This is to avoid OOM error.
                        sub_batch_size = 4 # 16 # 8 # 4
                        sub_batch_outputs = []
                        sub_batch_zs = []
                        for bs in range(32 // sub_batch_size):
                            bs_start = bs * sub_batch_size
                            bs_end = (bs+1) * sub_batch_size
                            cur_target = target[bs_start:bs_end]
                            cur_target_mask = target_mask[bs_start:bs_end]
                            cur_xt = inv_noise_detach[bs_start:bs_end].detach().clone()
                            cur_motion_length = model_kwargs['y']['lengths'][bs_start:bs_end]
                            model_kwargs_sub = copy.deepcopy(model_kwargs)
                            model_kwargs_sub["y"]["text"] = [""]
                            if scale != 1.:
                                model_kwargs_sub['y']['scale'] = torch.ones(sub_batch_size, device=model_device) * scale

                            loss_fn = CondKeyLocationsLoss(
                                target=cur_target,
                                target_mask=cur_target_mask,
                                motion_length=cur_motion_length,
                                transform=dataloader.dataset.t2m_dataset.transform_th,
                                inv_transform=dataloader.dataset.t2m_dataset.inv_transform_th,
                                abs_3d=args.abs_3d,
                                use_mse_loss=False,
                            ...
                            criterion = lambda x: loss_fn(x, **model_kwargs_sub)
                            solver = lambda z: diffusion.ddim_sample_loop_full_chain(
                                model,
                                (sub_batch_size, model.njoints, model.nfeats, self.max_motion_length),
                            ...
                            if DEBUG and bs == 0:
                                ...
                                for j in range(sub_batch_size): #  bs_start, bs_end):
                                    hist = out["hist"][bs_start + j]
                                    ...
                                    for key in [
                                        ...
                                        motion_id = f'{i:04d}_{t:02d}_{bs_start + j:02d}'
                                        plt.figure()
                                        if key == "loss":
                                        ...
                                        plt.legend([key])
                                        plt.savefig(os.path.join(self.save_dir, f"hist_{motion_id}_{key}.png"))
                                        plt.close()

                            sub_batch_outputs.append(final_out)
                            sub_batch_zs.append(out["z"])

                            if DEBUG:
                                break
                        
                        final_motion = torch.cat(sub_batch_outputs, dim=0)
                        final_z = torch.cat(sub_batch_zs, dim=0)
                        del sub_batch_outputs
                        del sub_batch_zs
                        del noise_opt
                    else:
                        # skip optimization use the inverted noise
                        final_z = inv_noise_detach
                    
                    # if log_motion:
                    #     cur_motion = sample_to_motion(final_motion, self.dataset, model, abs_3d=args.abs_3d)
                    #     plot_3d_motion(os.path.join(self.save_dir, f"{motion_id}_gen_opt.mp4"), self.dataset.kinematic_chain, 
                    #             cur_motion[log_idx_vis].permute(2,0,1).detach().cpu().numpy(), 'title', 'humanml', fps=20)

                    # 4. Inference again
                    if noise_opt_conf.last_unroll_steps is None or noise_opt_conf.last_unroll_steps == noise_opt_conf.unroll_steps:
                        last_step_diffusion = diffusion
                    else:
                        assert noise_opt_conf.last_unroll_steps == 100, f"last_unroll_steps must be 100, but got {noise_opt_conf.last_unroll_steps}"
                        last_step_diffusion = diffusion_ori
                    final_motion_full_inf = last_step_diffusion.ddim_sample_loop(
                        model,
                        (args.batch_size, model.njoints, model.nfeats, motion.shape[3]),
                        clip_denoised=not args.predict_xstart,
                        model_kwargs=model_kwargs,
                    ...
                    # save to file
                    # torch.save(final_motion_full_inf, batch_path)
                    if not DEBUG:
                        torch.save(final_motion, batch_path)

                    # if log_motion:
                    #     cur_motion = sample_to_motion(final_motion_full_inf, self.dataset, model, abs_3d=args.abs_3d)
                    #     plot_3d_motion(os.path.join(self.save_dir, f"{motion_id}_gen_opt_full.mp4"), self.dataset.kinematic_chain, 
                    #             cur_motion[log_idx_vis].permute(2,0,1).detach().cpu().numpy(), 'title', 'humanml', fps=20)
                    # import pdb; pdb.set_trace()

                sample = final_motion[:, :, :, :self.max_motion_length]
                ...
                # NOTE: To test if the motion is reasonable or not
                if log_motion:
                    for j in tqdm([0,1,2,3], desc="generating motion"):
                        motion_id = f'{i:04d}_{t:02d}_{j:02d}'
                        vis_len = model_kwargs['y']['lengths'][j].cpu().numpy()
                        cap = model_kwargs['y']['text'][j]
                        plot_3d_motion(os.path.join(self.save_dir, f"{motion_id}_motion_cond.mp4"), self.dataset.kinematic_chain, 
                            cur_motion[j].permute(2,0,1)[:vis_len].detach().cpu().numpy(), cap, 'humanml', fps=20)
                        plot_3d_motion(os.path.join(self.save_dir, f"{motion_id}_gt.mp4"), self.dataset.kinematic_chain, 
                            gt_skel_motions[j].permute(2,0,1)[:vis_len].detach().cpu().numpy(), cap, 'humanml', fps=20)
                        plot_3d_motion(os.path.join(self.save_dir, f"{motion_id}_first_gen.mp4"), self.dataset.kinematic_chain,
                            first_gen_motion[j].permute(2,0,1)[:vis_len].detach().cpu().numpy(), cap, 'humanml', fps=20)
