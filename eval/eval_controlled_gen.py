...
from data_loaders.humanml.motion_loaders.model_motion_loaders import get_pose_cond_loader, get_mdm_loader, get_mdm_loader_cond
from data_loaders.humanml.utils.metrics import *
from data_loaders.humanml.networks.evaluator_wrapper import EvaluatorMDMWrapper
from collections import OrderedDict
from data_loaders.humanml.scripts.motion_process import *
from data_loaders.humanml.utils.utils import *
from utils.model_util import create_model_and_diffusion, load_model_wo_clip, load_saved_model, create_gaussian_diffusion
...
if __name__ == '__main__':
    ...
    noise_opt_conf = NoiseOptOptions(
        ...
        decorrelate_scale=0, # 1e3, # 1e6 (for split grad), 100 (for combined grad)
        starting_noise_seed=0,
    )
    ...
    log_file = os.path.join(os.path.dirname(args.model_path),
                            'eval_{}_{}_{}'.format(task, name, niter))
    ...
    log_file += f'_opt{opt_steps}_unroll{noise_opt_conf.unroll_steps}'
    ...
    DEBUG = False
    if DEBUG:
        # this is just to make sure that the data loader is deterministic
        for i , (motion, model_kwargs) in enumerate(gen_loader):
            for j in range(len(model_kwargs['y']['text'])):
    ...
    args.ddim_step = 100 # Fix the base diffusion step to 100 # noise_opt_conf.unroll_steps
    ...
    model_dict = {"motion": model}
    diffusion_dict = {"motion": diffusion}
    ...
    eval_motion_loaders = {
        ...
        ################
        'vald':
        lambda seed: get_pose_cond_loader(
            args, model_dict, diffusion_dict, args.batch_size,
