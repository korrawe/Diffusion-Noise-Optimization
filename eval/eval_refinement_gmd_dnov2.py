...
def evaluate_matching_score(eval_wrapper, motion_loaders, file):
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        if motion_loader_name == "vald":
            ...
            print(
                f"---> [{motion_loader_name}] Skating Ratio: {skating_score:.4f}",
                file=file,
                flush=True,
            )
...
def evaluation(
    ...
    if joint_to_condition == "all":
        ...
        raise NotImplementedError(
            f"joint_to_condition [{joint_to_condition}] not implemented"
        )
    ...
    model_dataset = load_dataset(
        args, args.num_frames, split, hml_mode="eval", with_loader=False
    )
    ...
    full_dataset = load_dataset(
        eval_args, eval_args.num_frames, split, hml_mode="eval", with_loader=False
    )
    ...
    idx_loader = DataLoader(
        idx_dataset, batch_size=eval_args.batch_size, shuffle=False, num_workers=0
    )
    ...
    for (idx,), (motion_rep, model_kwargs) in tqdm(zip(idx_loader, gen_loader)):
        ...
        for i in idx:
            ...
            generated = sample_to_motion(
                generated.unsqueeze(0), model_dataset, model, abs_3d=args.abs_3d
            )
...
def run(
    ...
    if args.eval_project_num_joints == "all":
        ...
        save_dir = os.path.join(
            os.path.dirname(args.model_path), f"eval_{task}_{run_name}"
        )
    else:
        save_dir = os.path.join(
            os.path.dirname(args.model_path), f"eval_{task}", run_name
        )
