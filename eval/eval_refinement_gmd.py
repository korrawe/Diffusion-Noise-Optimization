...
import torch
from sample.condition import CondKeyLocations, CondKeyLocationsLoss, get_target_and_inpt_from_kframes_batch
...
def evaluation(
    ...
    # the motion dataset has abs and proj
    model_dataset = load_dataset(args, args.num_frames, split, hml_mode='eval', with_loader=False)

    # the full dataset has rel and no proj
    # this will be rel and no projection
    eval_args = copy.deepcopy(args)
    eval_args.use_random_proj = False
    eval_args.abs_3d = False
    
    full_dataset = load_dataset(eval_args, eval_args.num_frames, split, hml_mode='eval', with_loader=False)
    ...
    idx_loader = DataLoader(idx_dataset, batch_size=eval_args.batch_size, shuffle=False, num_workers=0)

    # create a dataset with only the indexes that need to be generated
    # mode = 'eval'  => we'll have mean, std of the abs 3d
    gen_loader = load_dataset(eval_args, eval_args.num_frames, split, hml_mode='eval', only_idx=idx_not_in_cache, shuffle=False, drop_last=False)
    ...
    diffusion_ori.data_get_mean_fn = full_dataset.t2m_dataset.get_std_mean
    diffusion_ori.data_transform_fn = full_dataset.t2m_dataset.transform_th
    diffusion_ori.data_inv_transform_fn = full_dataset.t2m_dataset.inv_transform_th
    ...
    for (idx,) , (motion_rep, model_kwargs) in tqdm(zip(idx_loader, gen_loader)):
        ...
        # assumes that the motion_rep is relative
        ...
        # load the target if if exists
        if os.path.exists(os.path.join(save_dir, target_motion_file(idx[0]))):
            target = []
            for i in range(len(idx)):
                target.append(torch.load(os.path.join(save_dir, target_motion_file(idx[i]))))
            target = torch.stack(target, dim=0)
            target_mask = torch.zeros_like(target, dtype=torch.bool)
            for j in range(batch_size):
                target_mask[j, :model_kwargs['y']['lengths'][j], target_joints, :] = True
        else:
            raise ValueError(f"the target file should always be loaded.")
            # Set target and mask
            target = gt_skels
            if NOISY_TARGET:
        ...
        def generate(target, target_mask):
            # target [bs, 196, 22, 3]
            # target_mask [bs, 196, 22, 3]
            batch_size = len(target)
            device = target.device

            # Prepare target
            # (opt_batch_size, n_keyframe)
            # sampled_keyframes =  ((max_frame - min_frame) * torch.rand(opt_batch_size, n_keyframe) + min_frame).long()
            # set all frames in 196 to be keyframes
            sampled_keyframes = torch.arange(0, 196).unsqueeze(0).repeat(batch_size, 1)
            dummy_skel_motions = torch.zeros([batch_size, 22, 3, n_frames])
            ...
            inpaint_motion_points, inpaint_mask_points) = get_target_and_inpt_from_kframes_batch(dummy_skel_motions, sampled_keyframes, full_dataset)
            ...
            model_kwargs['y']['inpainted_motion'] = inpaint_motion.to(device) # init_motion.to(device)
            model_kwargs['y']['inpainting_mask'] = inpaint_mask.to(device)

            model_kwargs['y']['inpainted_motion_second_stage'] = inpaint_motion_points.to(device)
            model_kwargs['y']['inpainting_mask_second_stage'] = inpaint_mask_points.to(device)
            ...
            cond_fn = CondKeyLocations(target=target,
                                        ...
                                        transform=model_dataset.t2m_dataset.transform_th,
                                        inv_transform=model_dataset.t2m_dataset.inv_transform_th,
                                        abs_3d=args.abs_3d,
            ...
            sample_motion = sample_fn_motion(
                            model,
                            (batch_size, model.njoints, model.nfeats, n_frames),  # motion.shape
            ...
            # this will be abs with proj
            return sample_motion

        final_out = generate(target, target_mask)
    ...
    gen_loader = load_dataset(eval_args, eval_args.num_frames, split, hml_mode='eval', only_idx=idx_to_generate, shuffle=False, drop_last=False)
    assert len(idx_loader.dataset) == len(gen_loader.dataset), f"idx_loader.dataset {len(idx_loader.dataset)} != gen_loader.dataset {len(gen_loader.dataset)}"
    for (idx,), (motion_rep, model_kwargs) in tqdm(zip(idx_loader, gen_loader)):
        ...
        for i in idx:
            ...
            generated = sample_to_motion(generated.unsqueeze(0), model_dataset, model, abs_3d=args.abs_3d)
    ...
    def calculate_fid():
        ...
        def load_differnt_to_motions(idx_to_generate):
            ...
            gen_loader = load_dataset(eval_args, eval_args.num_frames, split, hml_mode='eval', only_idx=other_idxs, shuffle=False, drop_last=False)
            motion_length = []
            motion = []
...
def run(num_samples_limit=10, batch_size=10, joints='all', noisy=True, debug=False, added_noise_level=0.01):
    ...
    if args.eval_project_num_joints == 'all':
        ...
        save_dir = os.path.join(os.path.dirname(args.model_path), f'eval_{task}_{run_name}')
    else:
        save_dir = os.path.join(os.path.dirname(args.model_path), f'eval_{task}', run_name)
...
if __name__ == '__main__':
    ...
    #     decorrelate_scale=1e4, # 2e6, 1e6 (for split grad), 100 (for combined grad),  1e3 (for unit grad, split)
    #     standardize_z_before_step=False,
    ...
    #     # lr_decay_steps=75,
    # )
    # run(noise_opt_conf, joints=task, noisy=noisy, num_samples_limit=5, debug=True)
    ...
    for num_limits in [300]:
        for task, noisy, added_noise in [
            ...
            run(num_samples_limit=num_limits, batch_size=100, 
    ...
    #         perturb_scale=0, # lr/100 best
    #         diff_penalty_scale=0,
    #         decorrelate_scale=0, # 2e6, 1e6 (for split grad), 100 (for combined grad),  1e3 (for unit grad, split)
    #         lr_scheduler=lr_scheduler,
    #         lr_decay_steps=lr_decay_steps,
    #         postfix="newx",
    #     )
    #     run(noise_opt_conf, num_samples_limit=300)

    # # find the best decorr scale for unit
    # for decorrelate_scale in [
    #     1e3, 
    #     2e3, 5e2, 
    #     # 5e3, 1e4,
    #     # 2e4, 5e4,
    ...
    #         perturb_scale=0, 
    #         diff_penalty_scale=0,
    #         decorrelate_scale=decorrelate_scale,
    #         postfix="newx",
    #     )
    #     run(noise_opt_conf, num_samples_limit=300)

    # # find the best decorr scale for sign(grad)
    # for decorrelate_scale in [
    #     5e5, 
    #     1e6, 
    #     2e6
    ...
    #         opt_steps=opt_steps,  #
    #         optimizer="adam", # adam
    #         grad_mode="sign", # sign, unit
    #         lr=lr,
    #         perturb_scale=0, 
    #         diff_penalty_scale=0,
    #         decorrelate_scale=decorrelate_scale, # 2e6, 1e6 (for split grad), 100 (for combined grad),  1e3 (for unit grad, split)
    #         postfix="newx",
    #     )
    #     run(noise_opt_conf, num_samples_limit=300)


    # # compare stadardization
    # for bf, af, decorr in [
    #     (True, False, 0),
    #     (False, True, 0),
    #     (True, False, 1e3),
    ...
    #             perturb_scale=0,
    #             diff_penalty_scale=0,
    #             decorrelate_scale=decorr,
    #             standardize_z_after_step=af,
    #             standardize_z_before_step=bf,
    #             postfix="newx",
    #         )
    #         run(noise_opt_conf, num_samples_limit=300)
    ...
    # # compare noise level after each step
    # for decorr_scale, perturb in [
    #     # (1e3, 0),
    #     (1e3, 1e4), 
    #     (1e3, 2e4), 
    #     (1e3, 5e4), 
    ...
    #         diff_penalty_scale=0,
    #         decorrelate_scale=decorr_scale, 
    #         postfix="newx",
    #     )
    #     run(noise_opt_conf, num_samples_limit=300)

    # # compare optimization steps
    # for opt_steps in [100,200,400]:
    #     noise_opt_conf = NoiseOptOptions(
    #         unroll_steps=10, # 10,  #
    #         opt_steps=opt_steps,  #
    #         optimizer="adam", # adam
    ...
    #         perturb_scale=0, 
    #         diff_penalty_scale=0,
    #         decorrelate_scale=1e3, # 2e6, 1e6 (for split grad), 100 (for combined grad),  1e3 (for unit grad, split)
    #         postfix="newx",
    #     )
    #     run(noise_opt_conf, num_samples_limit=300)
    ...
    #             grad_mode=grad_mode, 
    #             lr=lr,
    #             perturb_scale=lr/100,
    #             diff_penalty_scale=0,
    #             decorrelate_scale=1e3,
    #         )
    #         run(noise_opt_conf, num_samples_limit=300)

    # # find the best decorr scale for unit
    # for scheduler, length in [
    #     ("sgdr", 50), 
    #     ("sgdr", 75), 
    #     ("sgdr", 100), 
    # ]:
    ...
    #         lr_decay_steps=length,
    #         perturb_scale=lr/100, 
    #         diff_penalty_scale=0,
    #         decorrelate_scale=1e3,
    #     )
    #     run(noise_opt_conf, num_samples_limit=100)