...
IS_NOISY = True
JOINT_TO_CONDITION = 'all'
# HOME_DIR = "/home/konpat/motion-latent-diffusion/dno_eval_decorr0opt500_notrim/"
# HOME_DIR = "/home/konpat/motion-latent-diffusion/dno_eval_decorr0opt1000/"
HOME_DIR = "/home/konpat/motion-diffusion-model/save/gmd_final/eval_motion_projection_noise0.05_noiseopt500_unroll10_adam-0.9-0.999_lr0.05_warmup50_cosine500_noise0_gradunit_decorr1000.0_stop2_newx_bs60/"
LOAD_MLD = False
...
def plot_debug(motion_to_plot, name, gen_loader, length, clean_root=None):
    ...
    plot_3d_motion(name, gen_loader.dataset.kinematic_chain, 
                   motion_to_plot[:length].detach().cpu().numpy(), '', 'humanml',  # '' % length
                   fps=20, clean_root=clean_root)
...
def evaluation(
    ...
    joint_to_condition = JOINT_TO_CONDITION
    ...
    ##### Test Humor
    # # input_to_viz_file = "/home/korrawe/motion_gen/humor/out/amass_joints_noisy_fitting/results_out/Transitions_mocap_mazen_c3d_punchkarate_push_poses_205_frames_30_fps_17058932571/observations.npz"
    # input_to_viz_file = "/home/korrawe/motion_gen/humor/out/amass_joints_noisy_fitting/results_out/HumanEva_S3_ThrowCatch_3_poses_414_frames_30_fps_17059025500/observations.npz"
    
    # # input_to_viz_file = "/home/korrawe/motion_gen/humor/out/amass_joints_noisy_fitting/results_out/HumanEva_S2_Jog_3_poses_314_frames_30_fps_17055789121/observations.npz"
    # input_to_viz = np.load(input_to_viz_file)
    # results_dir = os.path.dirname(input_to_viz_file)
    # joints3d_input = torch.from_numpy(input_to_viz['joints3d'])

    # results_to_viz_file = "/home/korrawe/motion_gen/humor/out/amass_joints_noisy_fitting/results_out/HumanEva_S2_Jog_3_poses_314_frames_30_fps_17055789121/stage3_results.npz"
    
    # results_to_viz_file = os.path.join(os.path.dirname(input_to_viz_file), "stage3_results.npz")
    # results_to_viz = np.load(results_to_viz_file)
    # joints3d_result = torch.from_numpy(results_to_viz['joints3d'])

    # viz test
    # input_to_viz_file = "/home/korrawe/motion_gen/humor/out/dno_eval/input_noisy.npz"
    # input_to_viz_all = np.load(input_to_viz_file)
    # obs_data = dict()  # ['index', 'length', 'motion']
    # input_to_viz = input_to_viz_all['motion']

    # # import pdb; pdb.set_trace()
    # index_to_viz = 3998   # input_to_viz_all['index'][0]
    # input_to_viz = torch.from_numpy(input_to_viz)[0]

    # results_to_viz_file = "/home/korrawe/motion_gen/humor/out/dno_eval/noisy_" + joint_to_condition + "/" + gen_motion_file(index_to_viz)
    # # results_to_viz_file = "/home/korrawe/motion_gen/humor/out/dno_eval/noisy_all/motion_01382.pt"
    # results_dir = os.path.dirname(results_to_viz_file)
    # results_to_viz = torch.load(results_to_viz_file)
    # import pdb; pdb.set_trace()

    ##################
    # humor_out_dir = "/home/korrawe/motion_gen/humor/out/dno_eval/"
    humor_out_dir = HOME_DIR
    input_save_dir = humor_out_dir # "/home/korrawe/motion_gen/humor/out/dno_eval/"

    if NOISY_TARGET:
        humor_out_dir += "noisy_"
    else:
        humor_out_dir += "clean_"
    
    humor_out_dir += joint_to_condition + "/"
    save_dir = humor_out_dir
    log_file = os.path.join(humor_out_dir, "log.txt")

    # # joints3d_torch = torch.from_numpy(joints3d)
    # swap y and z
    # results_to_viz = results_to_viz[:, :, [0, 2, 1]]
    # joints3d_input_swap = torch.cat([joints3d_input[:, :, 0:1], joints3d_input[:, :, 2:], joints3d_input[:, :, 1:2]], dim=2)
    # joints3d_result_swap = torch.cat([joints3d_result[:, :, 0:1], joints3d_result[:, :, 2:], joints3d_result[:, :, 1:2]], dim=2)
    # import pdb; pdb.set_trace()
    # plot_debug(results_to_viz, osp.join(results_dir, f"out.mp4"), gen_loader, results_to_viz.shape[0])
    # plot_debug(input_to_viz, osp.join(results_dir, f"input.mp4"), gen_loader, input_to_viz.shape[0])
    # plot_debug(joints3d_input_swap, osp.join(results_dir, f"input.mp4"), gen_loader, joints3d_input_swap.shape[0])
    # plot_debug(joints3d_result_swap, osp.join(results_dir, f"humor_fit.mp4"), gen_loader, joints3d_result_swap.shape[0])
    # import pdb; pdb.set_trace()
    ...
    for (idx,) , (motion_rep, model_kwargs) in tqdm(zip(idx_loader, gen_loader)):
        ...
        if NOISY_TARGET:
            ...
            seeds = idx + 1
            # import pdb; pdb.set_trace()
            aa = generate_det_noise(target.shape, seeds=seeds).to(target.device) * added_noise_level
        ...
        length_list.append(model_kwargs['y']['lengths'])
        motion_list.append(target)


    # import pdb; pdb.set_trace()
    index_all = torch.cat(index_list, dim=0).detach().cpu().numpy()
    length_all = torch.cat(length_list, dim=0).detach().cpu().numpy()
    motion_all = torch.cat(motion_list, dim=0).detach().cpu().numpy()
    # save to file
    save_dict = { 
        'index' : index_all,
        'length' : length_all,
        'motion' : motion_all, # Need to swap y and z for humor # Need to change FPS to 30 for humor
    }

    SAVE_TARGET = False
    if SAVE_TARGET:
        if NOISY_TARGET:
            np.savez(os.path.join(input_save_dir, "input_noisy"), **save_dict)
        else:
            np.savez(os.path.join(input_save_dir, "input_clean"), **save_dict)
    # import pdb; pdb.set_trace()
    ...
    for (idx,), (motion_rep, model_kwargs) in tqdm(zip(idx_loader, gen_loader)):
        ...
        # load from files
        for gt_i, i in enumerate(idx):
            if not os.path.exists(os.path.join(save_dir, gen_motion_file(i))):
                # if USE_INPUT_WEHN_FAIL:
                #     print(f"Motion [{os.path.join(save_dir, gen_motion_file(i))}] does not exist, using input...")
                #     generated_motions.append(gt_skels[gt_i:gt_i+1])
                #     target_motions.append(gt_skels[gt_i:gt_i+1])
                #     gt_motions.append(gt_skels[gt_i:gt_i+1])
                #     motion_lengths.append(model_kwargs['y']['lengths'][gt_i:gt_i+1])
                #     continue
                # else:
                print(f"Motion [{os.path.join(save_dir, gen_motion_file(i))}] does not exist, skipping...")
                continue
            
            idx_to_generate_exist.append(i)
            gt_motions.append(gt_skels[gt_i:gt_i+1])
            motion_lengths.append(model_kwargs['y']['lengths'][gt_i:gt_i+1])
            ...
            # import pdb; pdb.set_trace()
            if LOAD_MLD:
                # Padding for MLD output
                generated_pad = torch.zeros_like(target)
                generated_pad[:generated.shape[0], :, :] = generated
                generated = generated_pad

            # if LOAD_MLD:
            #     import pdb; pdb.set_trace()
            #     aa = sample_to_motion(generated.unsqueeze(0), gen_loader.dataset, model, abs_3d=False)
            # convert the generated motion to skeleton
            # import pdb; pdb.set_trace()
            # For humor, do not need to convert to skeleton again
    ...
    idx_to_generate_exist = np.array(idx_to_generate_exist)
    # Only use existing motions
    idx_to_generate = idx_to_generate_exist
    ...
    if DEBUG:
        for ii in range(len(generated_motions)):
            ...
            plot_debug(target_clone, osp.join(save_dir, f"{motion_id}_target.mp4"), gen_loader, motion_lengths[ii], clean_root=clean_root)
            plot_debug(gt_motions[ii], osp.join(save_dir, f"{motion_id}_gt.mp4"), gen_loader, motion_lengths[ii])
    # import pdb; pdb.set_trace()

    SAVE_FOR_VIS = False # True
    if SAVE_FOR_VIS:
        # Refined motion
        import pdb; pdb.set_trace()
    ...
    def calculate_fid():
        def load_all_motion_from_filenames(filenames, load_mld=False):
            motion = []
            for filename in tqdm(filenames):
                loaded_data = torch.load(filename)
                if load_mld:
                    # Padding for MLD output
                    generated_pad = torch.zeros([196, 22, 3])
                    generated_pad[:loaded_data.shape[0], :, :] = loaded_data
                    if loaded_data.shape[0] < 196:
                        # Repeat the last frame
                        generated_pad[loaded_data.shape[0]:, :, :] = loaded_data[-1:, :, :]
                    loaded_data = generated_pad
                    # import pdb; pdb.set_trace()
                motion.append(loaded_data)
        ...
        gen_motion = load_all_motion_from_filenames(gen_filenames, load_mld=LOAD_MLD)
        # For Humor
        # swap y and z
        # gen_motion = gen_motion[:, :, :, [0, 2, 1]]
        gen_motion = convert_skel_to_motion_rep(gen_motion, full_dataset)
...
# def eval_humor():
#     results_to_viz_file = "/home/korrawe/motion_gen/humor/out/amass_joints_noisy_fitting/results_out/HumanEva_S2_Walking_1_poses_379_frames_30_fps_17053928300/observations.npz"
#     results_to_viz = np.load(results_to_viz_file)
#     joints3d = results_to_viz['joints3d']
#     import pdb; pdb.set_trace()
    ...
    for num_limits in [300]:
        for grad_mode, decorr, std_af, warm_up, scheduler, steps, perturb, sep_ode, unroll in [
            ...
            for task, noisy, added_noise in [
                ...
                run(noise_opt_conf, num_samples_limit=num_limits, batch_size=15, #  batch_size=30, 
