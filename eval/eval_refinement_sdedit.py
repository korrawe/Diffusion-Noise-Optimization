...
def evaluation(
    ...
    assert joint_to_condition == 'all', f"joint_to_condition [{joint_to_condition}] not implemented"
    ...
    def target_motion_file(i):
        if NOISY_TARGET:
            return f'/shared/konpat/motion-diffusion-model/save/mdm_avg/eval_motion_projection_noise0.05_noiseopt500_unroll10_adam-0.9-0.999_lr0.05_warmup50_cosine500_noise0_gradunit_decorr1000.0_stop2_newx_bs30/target_{i:05d}.pt'
        else:
            return f'/shared/konpat/motion-diffusion-model/save/mdm_avg/eval_motion_projection_noiseopt500_unroll10_adam-0.9-0.999_lr0.05_warmup50_cosine500_noise0_gradunit_decorr1000.0_stop2_newx_bs30/target_{i:05d}.pt'
    ...
    model, diffusion_ori = create_model_and_diffusion(args, gen_loader)
    ...
    for (idx,) , (motion_rep, model_kwargs) in tqdm(zip(idx_loader, gen_loader)):
        ...
        # convert target skeleton to motion representation
        current_x = convert_skel_to_motion_rep(target, full_dataset).float().to(device)

        if sdedit_t == 1000:
            final_out = diffusion_ori.p_sample_loop(
                model,
            ...
            # do sdedit for k times
            for k in range(sdedit_k):
                # add noise to a certain degree and treat it as the initial noise
                t = torch.tensor([sdedit_t] * batch_size, device=device)
                current_x = diffusion_ori.q_sample(current_x, t - 1)

                # denoise using p_sample 
                # loop denoising from t to 0
                indices = list(range(sdedit_t))[::-1]
                for i in tqdm(indices):
                    _t = torch.tensor([i] * batch_size, device=device)
                    with torch.no_grad():
                        sample_fn = diffusion_ori.p_sample if not ddim else diffusion_ori.ddim_sample
                        out = sample_fn(
                            model,
                            current_x,
                            _t,
    ...
    for (idx,), (motion_rep, model_kwargs) in tqdm(zip(idx_loader, gen_loader)):
        ...
        for i in idx:
            # load the target
            target = torch.load(target_motion_file(i))
    ...
    def calculate_fid():
        ...
        target_filenames = [target_motion_file(i) for i in idx_to_generate]
...
def run(sdedit_t, sdedit_k, ddim, num_samples_limit=10, batch_size=10, joints='all', noisy=True, debug=False, added_noise_level=0.01):
    run_name = f"sdedit_t{sdedit_t}_k{sdedit_k}"
    if ddim:
        run_name += "_ddim"
        ...
        for task, noisy, added_noise in [
            ...
            for k in [3]:
                for t in [
                    ...
                    run(sdedit_k=k, sdedit_t=t, ddim=False, num_samples_limit=num_limits, batch_size=100, 
