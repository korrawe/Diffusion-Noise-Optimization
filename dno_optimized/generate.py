...
def main(config_file: str, dot_list=None):
    ...
    start_from_noise, gen_sample, inv_noise, kframes, noise_opt_conf, obs_list, sample, sample_2, show_target_pose, target, target_mask = prepare_optimization(
    ...
    solver = lambda z: ddim_loop_with_gradient(
        diffusion,
        model,
...
def prepare_dataset_and_model(args):
    ...
    collate_args = [{
        "inp": torch.zeros(args.n_frames),
        "tokens": None,
        "lengths": args.gen_frames,
...
def prepare_optimization(args, data, diffusion, kframes, model, model_device, model_kwargs, obs_list, show_target_pose,
                         target):
    sample_2 = None
    if args.load_from == '':
    ...
    if args.task == "motion_blending":
        ...
        gen_sample = torch.cat(
            [
                ...
                sample[:, :, :, args.gen_frames // 2:],  # half from the first motion
                sample_2[
                :, :, :, NUM_OFFSET: args.gen_frames // 2 + NUM_OFFSET
    ...
    if first_row_is_sampled_with_limited_quality:
        diffusion = create_gaussian_diffusion(
            args, timestep_respacing=f"ddim{args.num_ode_steps}"
        )
    ...
    return start_from_noise, gen_sample, inv_noise, kframes, noise_opt_conf, obs_list, sample, sample_2, show_target_pose, target, target_mask
...
def process_results(args, data, gen_sample, inter_out, model, model_kwargs, out, sample, sample_2, step_out_list,
    ...
    if args.task == "motion_projection":
        ...
        noisy_motion = (
            target[0, :args.gen_frames, :, :].detach().cpu().numpy().transpose(1, 2, 0)
        )
