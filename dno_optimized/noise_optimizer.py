...
class DNO:
    ...
    def __call__(self, optimizer, num_steps: int = None):
        ...
        if optimizer == "Adam":
            ...
            return self.training_with_LBFGS()
            
        elif optimizer == "SGD":
            return  self.training_with_SGD()
    ...
    def training_with_SGD(self, num_steps, batch_size):
        optimizer = torch.optim.SGD([self.current_z], lr=self.conf.lr)
    ...
    def training_with_LBFGS(self, num_steps, batch_size):
        optimizer = torch.optim.LBFGS([self.current_z], history_size = 10, lr=self.conf.lr)
